...
/**
 * Smoke test headless: 4 jugadores confirman inicio por consenso.
 */
public class SmokeInicioPartida {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!lobby.await(10, TimeUnit.SECONDS)) {
            System.err.println("[SMOKE] FALLO: timeout lobby (4 jugadores)");
            System.exit(1);
        }

        for (String id : ids) {
            dispatcher.dispatch(HOST, BROKER, serializador.objetoABytes(
                new EventoIniciarPartida(id, "INICIAR_" + id)));
            Thread.sleep(200);
        }

        if (!partida.await(15, TimeUnit.SECONDS)) {
            System.err.println("[SMOKE] FALLO: timeout partida iniciada");
            System.exit(1);
        }
        ...
        System.out.println("[SMOKE] OK - Inicio por consenso con 4 jugadores funciona.");
        servidor.detener();
        System.exit(0);
    }
}
